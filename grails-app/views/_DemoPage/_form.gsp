<%@ page import="kickstart._DemoPage" %>
			...
			<div class="${hasErrors(bean: _DemoPageInstance, field: 'myDate2', 'error')} ">
				<label for="myDate2" class="control-label"><g:message code="_DemoPage.myDate2.label" default="My Date2" /></label>
				<div>
					<bs:datePicker name="myDate2" precision="day"  value="${_DemoPageInstance?.myDate2}" default="none" noSelection="['': '']" />
					<span class="help-inline">${hasErrors(bean: _DemoPageInstance, field: 'myDate2', 'error')}</span>
				</div>
			</div>

			<div class="${hasErrors(bean: _DemoPageInstance, field: 'myEnum', 'error')} ">
				<label for="myEnum" class="control-label"><g:message code="_DemoPage.myEnum.label" default="My Enum" /></label>
				<div>
					<g:select class="form-control" name="myEnum" from="${kickstart._DemoPage$Suit?.values()}" keys="${kickstart._DemoPage$Suit.values()*.name()}" value="${_DemoPageInstance?.myEnum?.name()}" noSelection="['': '']"/>
